...
export async function POST(
  ...
  try {
    ...
    if (!session || session.userType !== "org_admin" || !session.organizationId) {
      ...
      logRequestMeta("/api/org/providers/[id]/mfa/reset-recovery", requestId, status, Date.now() - started);
      return res;
    }
    ...
    status = 500;
    console.error("[org/providers/[id]/mfa/reset-recovery] POST Error", error);
