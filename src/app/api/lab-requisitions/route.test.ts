...
const endpoint = "http://localhost/api/lab-requisitions";
const baseSession = {
...
describe("POST /api/lab-requisitions", () => {
  beforeEach(() => {
  ...
  it("returns 200 and logs audit for same-org", async () => {
    ...
    expect(logPhysicianPhiAuditMock).toHaveBeenCalledWith(
      expect.objectContaining({ eventType: "lab_requisition_saved" }),
    );
  });
});

describe("GET /api/lab-requisitions", () => {
  beforeEach(() => {
    getCurrentSessionMock.mockResolvedValue(baseSession);
    loadSessionAccessScopeMock.mockResolvedValue({
      physicianId: "22222222-2222-4222-8222-222222222222",
  ...
  it("returns 200 and logs audit for same-org", async () => {
    ...
    expect(response.status).toBe(200);
    expect(logPhysicianPhiAuditMock).toHaveBeenCalledWith(
      expect.objectContaining({ eventType: "lab_requisition_viewed" }),
    );
  });
});

describe("DELETE /api/lab-requisitions", () => {
  beforeEach(() => {
    getCurrentSessionMock.mockResolvedValue(baseSession);
    loadSessionAccessScopeMock.mockResolvedValue({
      physicianId: "22222222-2222-4222-8222-222222222222",
  ...
  it("returns 401 when unauthenticated", async () => {
    getCurrentSessionMock.mockResolvedValueOnce(null);
    const response = await DELETE(new Request(`${endpoint}?code=session-1&id=req-1`) as any);
    expect(response.status).toBe(401);
  });
  ...
  it("returns 200 and logs audit for same-org", async () => {
    const response = await DELETE(
      new Request(`${endpoint}?code=session-1&id=req-1`, {
        headers: { "user-agent": "vitest" },
      }) as any,
    );
    expect(response.status).toBe(200);
    expect(logPhysicianPhiAuditMock).toHaveBeenCalledWith(
      expect.objectContaining({ eventType: "lab_requisition_deleted" }),
    );
  });
});
