...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!session || session.userType !== "super_admin") {
      ...
      logRequestMeta("/api/admin/sessions/terminate", requestId, status, Date.now() - started);
      return res;
    }
    ...
    if (scope === "all") {
      result = await query("DELETE FROM physician_sessions");
    } else if (scope === "user" && isUuid(userId)) {
      result = await query("DELETE FROM physician_sessions WHERE user_id = $1", [userId]);
    } else {
      status = 400;
      const res = NextResponse.json(
    ...
    status = 500;
    console.error("[admin/sessions/terminate] Error:", error);
