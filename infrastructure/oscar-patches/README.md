...
the "Create consultation" button), and `eform-fax/patch_rx_prefill.py` for the
Rx3 pad (`oscarRx/SearchDrug3.jsp`, the "Create prescription" button — stages
dictated drugs via OSCAR's own search/createNewRx endpoints with a strict
name+strength match gate). Those two are webapp JSPs, so a WAR redeploy DOES
wipe them. Full field maps, format, and smoke test:
