...
function rosterStatusChangedNotBlank() {
	// Enrolment status is optional - only run status-specific validation when a status is actually chosen.
	if (rosterStatusChanged() && document.updatedelete.roster_status.value!="") {
		return true;
	}
	return false;
}
...
function rosterStatusDateAllowed() {
	// Enrolment date is allowed even when the enrolment status is left blank.
	return true;
}
...
<td style="width:100%"><b><%=providerBean.getProperty(demographic.getProviderNo()==null?"":demographic.getProviderNo(),"")%></b></td></tr>
...
function parsedate_joined(){
    ...
    console.log("joined="+year+"-"+month+"-"+day);
        ...
        <div class="control-group span5" style="display:none"><%-- MyMD 2026-08-24: enrollment date hidden; input kept so stored values still POST --%>
